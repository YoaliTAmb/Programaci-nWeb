...
if(!empty($nombre) || !empty($password) || !empty($email) || !empty($telefono) || !empty($genero) || !empty($estado)){
    ...
    if (mysqli_connect_error()){
        ...
        $SELECTN = "SELECT usuario from data where usuario = ?";
        $SELECT = "SELECT telefono from data where telefono = ? limit 1"; 
        
        $INSERT = "INSERT INTO data (usuario, contraseña, email, telefono, genero, entidad) values(?,?,?,?,?,?)";
